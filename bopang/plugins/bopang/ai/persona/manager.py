...
from ...config import AISSETTING_DIR
...
注意：用户id为3197425473的用户是Master(美浦波旁的专属训练员)，其余用户也是特雷森学院的训练员，需要尊敬。
"""
...
class PersonaManager:
    ...
    def _init_persona(self):
        """初始化：检查文件夹，如果没有文件则创建默认文件"""
        files = list(self.persona_dir.glob("*.txt"))
        if not files:
            default_file = self.persona_dir / "bourbon.txt"
            try:
...
async def handle_persona_list(bot,event, ):
    ...
    if not personas:
        ...
        for p in personas:
            ...
            msg += f"  {indicator} `{p}`\n"
    msg += f"\n当前加载: **{current}**"
    await bot.send(event, msg, reply=True)


async def handle_switch_persona(bot, event, ):
    args = event.get_message().extract_plain_text().strip()
    if args.startswith("/切换人设"):
        args = args[5:].strip()

    if not args:
        await bot.send(event, "⚠️ 请输入人设文件名（无需.txt后缀）", reply=True)
        return

    if persona_manager.load_persona(args):
        await bot.send(event, f"✅ 人设已切换为: **{args}**\n波旁已重置性格模块~", reply=True)
    else:
        await bot.send(event, f"❌ 切换失败。未在 `data/aisetting` 中找到文件 `{args}.txt`", reply=True)
