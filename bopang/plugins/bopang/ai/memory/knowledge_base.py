...
class KnowledgeBaseMemory:
    ...
    def _init_vectorstore(self) -> FAISS:
        ...
        if os.path.exists(index_file) and os.path.exists(pkl_file):
            try:
                ...
                logger.warning(f"加载向量数据库失败，将创建新数据库: {e}")
    ...
    def search_knowledge(
        ...
        if score_threshold is not None:
            ...
            docs = [doc for doc, score in docs_with_scores if (1 - score) >= score_threshold]
        else:
            docs = self.vectorstore.similarity_search(query, k=k, filter=metadata_filter)

        results = [doc for doc in docs if not doc.metadata.get("__placeholder__")]
        if not results:
            return "未找到相关知识"
