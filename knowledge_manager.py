...
- 绕过 bopang 包的 __init__.py 链，避免触发 NoneBot 插件注册
...
# 向上查找包含 pyproject.toml 且 name="bopang" 的项目根目录
project_root = None
...
for _ in range(10):
    ...
    if toml_path.exists():
        try:
            ...
            if 'name = "bopang"' in content or "name = 'bopang'" in content:
...
if not project_root:
    print("\u274c 严重错误：未找到 bopang 项目根目录！")
    print("请确认此脚本放置在 bopang 项目目录内。")
    input("\n按回车键退出...")
    sys.exit(1)
...
#   bopang.plugins.bopang.config 在 sys.modules 中
# 以及所有中间包在 sys.modules 中

# 1. 创建 stub 包（不执行 __init__.py，不触发 NoneBot）
plugin_dir = project_root / "bopang" / "plugins" / "bopang"
ai_dir = plugin_dir / "ai"
memory_dir = ai_dir / "memory"

_create_stub_package("bopang", [str(project_root / "bopang")])
_create_stub_package("bopang.plugins", [str(project_root / "bopang" / "plugins")])
_create_stub_package("bopang.plugins.bopang", [str(plugin_dir)])
_create_stub_package("bopang.plugins.bopang.ai", [str(ai_dir)])
_create_stub_package("bopang.plugins.bopang.ai.memory", [str(memory_dir)])

# 2. 直接加载 config.py
try:
    config_module = _load_module_direct(
        "bopang.plugins.bopang.config",
        plugin_dir / "config.py",
        package_name="bopang.plugins.bopang",
    )
...
try:
    kb_module = _load_module_direct(
        "bopang.plugins.bopang.ai.memory.knowledge_base",
        memory_dir / "knowledge_base.py",
        package_name="bopang.plugins.bopang.ai.memory",
    )
    KnowledgeBaseMemory = kb_module.KnowledgeBaseMemory
except Exception as e:
    print(f"\u274c 加载 knowledge_base.py 失败: {e}")
    print("请确认新版插件架构文件完整：")
    print("  - bopang/plugins/bopang/config.py")
    print("  - bopang/plugins/bopang/ai/memory/knowledge_base.py")
    input("\n按回车键退出...")
    sys.exit(1)
